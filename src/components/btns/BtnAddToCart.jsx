...
function BtnAddToCart({ product, size, count }) {
    ...
    function addCard() {
        if (selectedList?.some((item) => item.id === product.id)) {
            ...
            if (size.current === null) {
                ...
                setTimeout(() => {
                    ...
                    }).then((response) => response.json().then((res) => { setInnerText('added to cart'); setSelect(res);className.current = 'w-[127px] bg-green border border-green text-white font-medium rounded-[31.5px] mr-3' }))
                }, 2000)
